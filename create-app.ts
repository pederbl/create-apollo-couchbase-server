...
export async function createApp({
  ...
  console.log(`Creating a new Apollo On The Couch server in ${chalk.green(root)}.`)
  console.log()
  ...
  if (example) {
    ...
    // Copy `.env` if the application did not provide one
    const envPath = path.join(root, '.env')
    console.log("Template .env file", getTemplateFile({ template, mode, file: '.env' }))
    if (!fs.existsSync(envPath)) {
      fs.copyFileSync(
        getTemplateFile({ template, mode, file: '.env' }),
        envPath
      )
    }
    
    // Copy `apollo-couchbase-env.d.ts` to any example that is typescript
    const tsconfigPath = path.join(root, 'tsconfig.json')
    if (fs.existsSync(tsconfigPath)) {
      fs.copyFileSync(
        getTemplateFile({ template, mode: 'ts', file: 'apollo-couchbase-env.d.ts' }),
        path.join(root, 'apollo-couchbase.d.ts')
      )
    }
  ...
  if (hasPackageJson) {
    ...
    console.log('    Builds the server for production.')
    console.log()
    console.log(chalk.cyan(`  ${packageManager} start`))
    console.log('    Runs the built server in production mode.')
    console.log()
